...
RUN /opt/miniconda/bin/conda create -n morld python=3.7.6 -y  \
&& /opt/miniconda/bin/conda install -y -n morld networkx=2.4 numpy=1.18.1 absl-py=0.9.0 pandas=1.0.1  cmake openmpi scipy matplotlib \
&& /opt/miniconda/bin/conda install -y -n morld openbabel=2.4.1 rdkit=2018.09.1 -c conda-forge \
...
COPY optimize_BE.py ./
COPY config.txt ./
COPY 3zosA_prepared.pdbqt ./
COPY 3zosA_prepared.pdb ./
...
ENV PATH "/opt/conda-env/bin:/opt/miniconda/envs/morld/bin/:/var/lang/bin:/usr/local/bin:/usr/bin:/bin:/opt/bin"
ENV OUTPUT_DIR "./save"
